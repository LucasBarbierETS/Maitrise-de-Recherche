        ...
        function obj = classMPPSBH_Rectangular_iter2(config)
            ...
            if nargin > 0  && ~isempty(config) && length(fields(config)) > 3
                ...
                for i = 1:length(pp)
                    ...
                    % Cavité trapezoidale
                    % obj.Configuration.ListOfSubelements{end+1} =
                    % classcavity_rectangular(classcavity_rectangular.create_config(ct(i)/2, mpw(i), mpd(i)));
                    obj.Configuration.ListOfSubelements{end+1} = classcavity_trapezoidal_subdiv(classcavity_trapezoidal_subdiv.create_config(ct(i)/2, mpw(i), mpd(i), wc, dc, 2));
                    ...
                    obj.Configuration.ListOfSubelements{end+1} = classcavity_trapezoidal_subdiv(classcavity_trapezoidal_subdiv.create_config(ct(i)/2, wc, dc, mpw(i+1), mpd(i+1), 2));
                end 
            end
        end
    end
