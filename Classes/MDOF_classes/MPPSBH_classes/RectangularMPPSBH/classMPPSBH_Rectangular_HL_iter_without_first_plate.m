classdef classMPPSBH_Rectangular_HL_iter_without_first_plate < classMPPSBH_Rectangular
    ...
    methods
        function obj = classMPPSBH_Rectangular_HL_iter_without_first_plate(config)
            ...
            for i = 1:length(pppp)
                obj.Configuration.ListOfObjects{i} = Cell_MPPSBHr_HL_iter(Cell_MPPSBHr.create_config(pppp(i), phr(i), pt(i), ...
                ct(i), cavd, cavw, mpw(i), mpw(i+1), mpd(i), mpd(i+1), cm{i}));

                % obj.Configuration.ListOfObjects{i} = Cell_MPPSBHr_HL_iter(Cell_MPPSBHr.create_config(prp(i), phr(i), pt(i), ...
                % ct(i), cavd, cavw, mpw(i), mpw(i+1), mpd(i), mpd(i+1), cm(i))); 
            end 
        end
    end
