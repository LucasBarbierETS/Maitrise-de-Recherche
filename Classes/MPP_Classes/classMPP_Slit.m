classdef classMPP_Slit < classJCA
...
%               - scalar : la corection de longueur est donnée et utilisée telle quelle
    ...
    properties
      ...
      % propriétés héritées de classJCA
    ...
    methods 
        ...
        function obj = classMPP_Slit(config)

                sl = config.SlitsLength;
                ...
                if isnan(config.CorrectionLength)
                    cl = 2 * 0.48 * sqrt(sl * sw); % [2] p.6 eq. 4  a revoir
                end

                % Résistivité
                sig = @(eta) 4 * k0 * eta / pp / rh^2; % [1] p.5 eq. 11, 12
    
                obj@classJCA('rigid', pp, sig, 1, rh, rh, pt + cl); % correction dans la longueur
        ...
        function ep = equivalent_parameters(obj, air, w) % on définie des paramètres de poreux équivalent
            ...
            obj.AirflowResistivity = obj.AirflowResistivity(air.parameters.eta); % ??? je ne sais pas ce que ca fait
