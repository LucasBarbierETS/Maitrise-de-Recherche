...
%% ========================================================================
    ...
    methods
        ...
        function [TM, options] = transfer_matrix(obj, env, options)
            ...
            obj.Configuration.AirFlowResistivity = ...
                @(env, options) obj.compute_resistivity(env, obj.Configuration, options);

            obj.Configuration.Tortuosity = ...
                @(env, options) obj.compute_tortuosity(env, obj.Configuration, options);
            
            [TM, options] = transfer_matrix@classJCA_Rigid(obj, env, options);
        end
    end
    ...
    methods (Static, Access = public) % Physique
        ...
        function sig = compute_resistivity(env, config, options)
            ...
            % Vitesse RMS
            if strcmp(options.HL_method, 'all')
                v_rms = abs(options.u_ext/(s*sqrt(2)));
            
            elseif strcmp(options.HL_method, 'retropropagation') || (strcmp(options.HL_method, 'first') && perso_isones(options.Id))
                v_rms = abs(options.u_in/(s*sqrt(2)));
            else
                v_rms = 0;
            end

            % Nombre de Mach
            if strcmp(options.Flow_method, 'with') && perso_isones(options.Id)
                M = options.M;
            else
                M = 0;
            end
            ...
            + beta * env.air.parameters.rho * (1 - phi^2) / (pi * t * phi * Cd^2) * v_rms ...
            + env.air.parameters.Z0 * (1 - phi^2) / (phi * t) * q * M;
        end
        ...
        function tor = compute_tortuosity(env, config, options)
            ...
            tor =  1 + 2 * psi ./ (t * (1 + v_rms / (phi * env.air.parameters.c0))) ...
                * 0.48 * sqrt(pi * pr^2) * sum_a / (1 + 305 * M^3);
        end
    end
    ...
    methods (Static, Access = public) % Validation
        ...
        function validate(handle_env)
            ...
            plot(env.w / (2*pi), E1.absorption_coefficient(env), 'Color', 'b', 'LineWidth', 1, 'DisplayName', 'Modèle analytique');
            ...
            plot(env.w / (2*pi), E2.absorption_coefficient(env), 'Color', 'b', 'LineWidth', 1, 'DisplayName', 'modèle foam 1');
            % plot(env.w / (2*pi), E2_2.absorption_coefficient(env), 'Color', 'r',
            ...
            % hold on
            % subtitle("Atalla2007 - fig. 3 - p. 9")
            % plot(env.w / (2*pi), E1.absorption_coefficient(env), 'Color', 'b', 'LineWidth', 1, 'DisplayName', 'Modèle analytique - classMPP_Circular');
        ...
        function validate_HL(handle_env)
            ...
            plot(env.w/(2*pi), E.absorption_coefficient(env, 'iter'), 'DisplayName', 'Modèle non-linéaire itératif');
            ...
            plot(env.w/(2*pi), E.absorption_coefficient(env, 'iter'), 'DisplayName', 'Prédiction du modèle fort niveau');
            ...
            E = classelement(classelement.create_config({plate, cavity}, 'closed', S));
            plot(env.w/(2*pi), E.absorption_coefficient(env), 'DisplayName', 'Prédiction du modèle linéaire');
            plot(env.w/(2*pi), E.absorption_coefficient(env, 'iter Laly'), 'DisplayName', 'Prédiction du modèle fort niveau');
        ...
        function validate_flow(handle_env)
            ...
            cavity = classcavity(classcavity.create_config(S, D));
            E = classelement(classelement.create_config({plate, cavity}, 'closed', S));
            plot(env.w/(2*pi), E.absorption_coefficient(env, 'iter'), 'DisplayName', 'Prédiction du code HL avec écoulement');
