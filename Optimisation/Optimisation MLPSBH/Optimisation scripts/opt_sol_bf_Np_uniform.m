...
x0_to_MPPSBH_i = @(x0, i) classMPPSBH_Rectangular(classMPPSBH_Rectangular.create_explicit_config(6, cavities_depth, cavities_width, ...
...
cost_function = @(x0, env) sum(((params_to_MPPSBH_assembly(x0).absorption_coefficient(env) - g_bf(env)) .* (g_bf(env) > 0.1)).^2);
...
%     sum( ...
%         ((params_to_MPPSBH_assembly(x0).absorption_coefficient(env) - g_bf(env)) ... % calcul des écart à l'absorption idéale
...
% Affichage graphique
figure()
plot(env.w/(2*pi), g_bf(env) ,"--", env.w/ (2*pi), assembly_bf_opti.absorption_coefficient(env));
xlim([0 2000]);
