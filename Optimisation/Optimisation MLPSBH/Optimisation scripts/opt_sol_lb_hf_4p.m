root = 'E:\Montréal 2023 - 2025\Maitrise LB\MATLAB\Optimisation\Optimisation MLPSBH\';
foldername = '23-May-2025 - optimisation échantillon 60 mm 4 plaques\'; % A modifier

% Ouverture de l'environnement sauvegardé
load([root, foldername, 'environnement matlab.mat'])
...
NV = 4; % Nombre de variables pour chaque solutionlau
...
dw_init = reshape(5 * eval_r(r_init), N, 1, NS, NP);
...
ub = repmat([2, 8 * eval_r(1), 12, 12], N, 1, NS); 
...
g_lb_hf = @(env) (env.w / (2*pi) > f_min_bf & env.w / (2*pi) < f_max_hf);
...
% --- Création du sous-élément simulé (ex. : réponse COMSOL) ---
...
% --- Ancienne version : Assemblage global uniquement à partir des MPPSBH
...
cost_function_lb_hf = @(x0, env) sum(((x0_to_global_assembly(x0).absorption_coefficient(env) - g_lb_hf(env)) .* (g_lb_hf(env) > 0.1)).^2);
...
% handle_plot_alpha = @(x, ~, ~) perso_plot_alpha(params_to_MPPSBH_assembly(reshape(x, N, NV, NS)).absorption_coefficient(env), ...
%                                           env, ...
%                                           g_lb_hf);
...
figure()
hold on
plot(env.w/(2*pi), g_lb_hf(env) , "--", 'DisplayName', 'Gabarit');
plot(env.w/ (2*pi), assembly_lb_hf_opti.absorption_coefficient(env), 'DisplayName', 'Assemblage');
...
% 
% MPPSBH_lb_hf_1.export_DXF(strcat(root, foldername, 'Fichiers de découpe laser'));

% % Enregistrement de l'environnement
%
% save(strcat(root, foldername, 'environnement matlab.mat'));