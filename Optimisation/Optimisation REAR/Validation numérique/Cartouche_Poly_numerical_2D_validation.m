function model = Cartouche_Poly_numerical_2D_validation(Cartouche_Poly, solN, env, varargin)

% Ce script/ fonction sert de validation pour le processus d'optimisation global de
% la cartouche acoustique ETS
    ...
    % On ajoute les élements contenus dans l'élement assembly. Comme on est
    ...
    model = HR.set_COMSOL_2D_Model(model, 2, ['bndt+' num2str(Poly.Configuration.Width) '+2*bndt'], '-tpt-agt', env);
    model = solN.set_COMSOL_2D_Model(model, 3, ['bndt+' num2str(Poly.Configuration.Width)  '+2*bndt+' num2str(HR.Configuration.Width) '+2*bndt'], '-tpt-agt', env);
    ...
    % Sauvergarde
    if nargin > 2
        mphsave(model, [varargin{1}, '\', varargin{2}, '.mph']);
    end

    % sol1.runAll;   

    % % Importation du modèle résolu
    ...
    mphsave(model, 'C:\Users\lucas.barbier\Documents\Maitrise dossier secondaire\MATLAB\Optimisation\Optimisation REAR\validation_Cartouche_Poly.mph')
