...
% local_root = uigetdir();
% load([local_root, '\environnement matlab.mat'])
...
Frequences.f_min_lb = 220;
Frequences.f_max_lb = 950;
...
cavities_thickness = cavities_total_thickness / N;
...
phi_min = 0.01;
phi_max = 0.20;
% lb = horzcat(repmat(r_min, 1, N * NS), repmat(nb_w_min, 1, N * NS), repmat(theta_min, 1, N * NS)); %#ok<RPMT1> % NV * N * NS
lb = horzcat(repmat(r_min, 1, N * NS), repmat(nb_w_min, 1, N * NS)); %#ok<RPMT1> % NV * N * NS
...
nb_w_max = 25;
% theta_max = 3;
% ub = horzcat(repmat(r_max, 1, N * NS), repmat(nb_w_max, 1, N * NS), repmat(theta_max, 1, N * NS)); % NV * N * NS
ub = horzcat(repmat(r_max, 1, N * NS), repmat(nb_w_max, 1, N * NS)); % NV * N * NS

% Valeurs initiales 

% r_init = randi([r_min, r_max], NP, N * NS);
init.r_init = r_min + (r_max - r_min) * rand(Ngen, N * NS);
init.nb_w_init = randi([nb_w_min, nb_w_max], Ngen, N * NS); 
% theta_init = theta_min + (theta_max - theta_min) * rand(Ngen, N * NS);
% x0 = horzcat(r_init, nb_w_init, theta_init);
init.x0 = horzcat(init.r_init, init.nb_w_init);

% Contrainte sur les variables entières

% intcon = find(horzcat(zeros(1, N * NS), ones(1, N * NS), zeros(1, N * NS)));
intcon = find(horzcat(zeros(1, N * NS), ones(1, N * NS)));
...
slit_width_hdl = @(radius, nb_width, theta) (nb_width - 1) .* 4 .* radius + 2 .* radius;
slit_depth_hdl = @(radius, nb_width, theta) (2 * depth_holes_number + 1) * radius;
...
% slit_width_cstr = @(M) slit_width_hdl(M(1,:), M(2,:), M(3,:)) - cavities_width; % slit_width < cavities_width
% slit_depth_cstr = @(M) slit_depth_hdl(M(1,:), M(2,:), M(3,:)) - cavities_depth; % slit_width < cavities_width
...
% real_porosity_inf_cstr = @(M) phi_min - real_porosity_hdl(M(1,:), M(2,:), M(3,:)); % phi > phi_min
% real_porosity_sup_cstr = @(M) real_porosity_hdl(M(1,:), M(2,:), M(3,:)) - phi_max; % phi < phi_max

% slit_width_cstr = @(M) slit_width_hdl(radius_m(M(1,:)), M(2,:), M(3,:)) - cavities_width; % slit_width < cavities_width
slit_width_cstr = @(M) slit_width_hdl(M(1,:), M(2,:)) - cavities_width; % slit_width < cavities_width
slit_depth_cstr = @(M) slit_depth_hdl(M(1,:), M(2,:)) - cavities_depth; % slit_width < cavities_width
% real_porosity_inf_cstr = @(M) phi_min - real_porosity_hdl(radius_m(M(1,:)), M(2,:), M(3,:)) - phi_min; % phi > phi_min
% real_porosity_sup_cstr = @(M) real_porosity_hdl(radius_m(M(1,:)), M(2,:), M(3,:)) - phi_max; % phi < phi_max
real_porosity_inf_cstr = @(M) phi_min - real_porosity_hdl(M(1,:), M(2,:)); % phi > phi_min
real_porosity_sup_cstr = @(M) real_porosity_hdl(M(1,:), M(2,:)) - phi_max; % phi < phi_max

neq_cstr = {slit_width_cstr, slit_depth_cstr, real_porosity_inf_cstr, real_porosity_sup_cstr};
eq_cstr = {};
...
%% =========================================================================
%%
%% Filtrage des configurations valides
%%
%% =========================================================================

fprintf("Filtrage des configurations valides parmi %d candidats...\n", Ngen);

% --- Extraction vectorisée des M = (r ; nb_w ; theta) ---
% x0 est Ngen × (N*NS*NV)
% On reforme M sous forme 3 × (N*NS) × Ngen
init = {};
init.M = permute( reshape(init.x0, Ngen, N * NS, NV), [3 2 1]);  % -> NV × nVars × Ngen

% Aplatir sur les plaques et les candidats d’un coup
init.Mflat = reshape(init.M, NV, []);   % -> NV × (Ngen*nVars)

init.r_all   = init.Mflat(1,:);
init.nbw_all = init.Mflat(2,:);
% init.theta_all = Mflat(3,:);

% --- Contraintes non linéaires sur chaque plaque ---
% init.ok_slit     = slit_width_hdl(init.r_all, init.nbw_all, init.theta_all) <= cavities_width;
% init.ok_depth    = slit_depth_hdl(init.r_all, init.nbw_all, init.theta_all) <= cavities_depth;
% init.ok_phi_min  = real_porosity_hdl(init.r_all, init.nbw_all, init.theta_all) >= phi_min;
% init.ok_phi_max  = real_porosity_hdl(init.r_all, init.nbw_all, init.theta_all) <= phi_max;

init.ok_slit     = slit_width_hdl(init.r_all, init.nbw_all) <= cavities_width;
init.ok_depth    = slit_depth_hdl(init.r_all, init.nbw_all) <= cavities_depth;
init.ok_phi_min  = real_porosity_hdl(init.r_all, init.nbw_all) >= phi_min;
init.ok_phi_max  = real_porosity_hdl(init.r_all, init.nbw_all) <= phi_max;

% Combine : toutes les contraintes doivent être vraies
init.plate_ok = init.ok_slit & init.ok_depth & init.ok_phi_min & init.ok_phi_max;

% Reshape : chaque colonne = un candidat
init.plate_ok = reshape(init.plate_ok, N * NS, Ngen);

% Un candidat est valide seulement si TOUTES ses plaques le sont
init.candidate_ok = all(init.plate_ok, 1);

valid_idx = find(init.candidate_ok);

fprintf("Configurations valides trouvées : %d / %d\n", numel(valid_idx), Ngen);

% Si aucune configuration valide
if isempty(valid_idx)
    error("Aucune configuration valide trouvée. Augmenter Ngen ou vérifier les contraintes.");
end

% On garde TOUTES les configurations valides
x0_final = init.x0(valid_idx, :);
clear('init');

fprintf("Nombre final de configurations valides conservées : %d\n", size(x0_final, 1));
...
%         cavities_depth, ...
%         cavities_width, ...
%         num2cell(config(i, :, 1)), ...                 % radius
%         num2cell(config(i, :, 1) * 4), ...             % dw
%         repmat({depth_holes_distance}, 1, N),... 
%         repmat({depth_holes_number}, 1, N),  ...
%         num2cell(config(i, :, 2)), ...                 % pw
%         repmat({plate_thickness}, 1, N), ...
...
objective = @(x) ...
    handle_objective(x, env, {g_obj_h1, g_obj_h2, g_obj_h3, g_obj_h4});
...
%     'Display', 'iter', ...
%     'PopulationSize', NP, ...
%     'InitialPopulationMatrix', x0_final, ...
%     'FunctionTolerance', 1e-1, ...
%     'ConstraintTolerance', 1e-6, ...
...
selected_xopti = sorted_xopti(selected_index, :);
...
for i = 1:length(selected_index)
    ...
    for j = 1:NS
        ...
        save([sub_sub_folder_name, '\MPPSBH_', num2str(j), '_config_', num2str(selected_index(i)), '.mat'], 'MPPSBH')
...
% 
% for i = 1:NS
%     Objets.MPPSBH_i(x_ETS(x_opti), radius(x_radius(x_opti)), i).export_report([report_root, '\rapport de configuration - MPPSBH ', num2str(j), '.xlsx'])
% end
