...
% Choix des objectifs
% objective = @(x) cell2mat(handle_objective(x, env, {g_obj_h1, g_obj_h3, g_obj_4, g_obj_lb}));
...
xopti_to_cell_array_of_Zs = @(x, env) arrayfun(@(i) vertcat(Objects.assembly_of_MPPSBH(xopti).surface_impedance(env)/env.air.parameters.Z0), ...
                                                    1:size(x, 1), 'UniformOutput', false);
