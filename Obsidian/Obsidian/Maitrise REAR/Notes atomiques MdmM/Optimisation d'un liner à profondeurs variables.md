...
Cet article se place de contexte de la [[réduction de bruit en aéronautique]]. Il s'intéresse à l'optimisation d'un [[liner acoustique pour le carénage d'un rotor]] dans le cadre du projet de taxi-volant [[Revolutionary Vertical Lift Technology (RVLT)]] développé par la NASA.
...
Les configurations optimisées sont modélisées avec un [[modèle réaliste de l'impédance de surface d'un liner]] des configurations tenant compte des forts niveaux. L'impédance de surface est alors utilisée pour résoudre l'[[équation de Helmholtz convectée en 2D]] dans un code de propagation par éléments finis.

Finalement les configurations sont imprimées en 3D par [[stéréolithographie]] est testée dans un [[tube d'impédance à écoulement rasant]]. Leur impédance est calculée par la [[méthode de Prony]] et comparée avec les résultats analytiques.

#### Protocole d'optimisation

##### Fréquences cibles

Les [[fréquences cibles pour l'optimisation]] sont la [[fréquence de passage des pâles]]  ($f_b \approx 200 Hz$) et ses [[harmoniques]] qui contribuent fortement un niveau d'émission sonore globale du véhicule.
...
L'optimisation porte sur 5 longueurs associées aux profondeurs des cavités. Le liner est la répétition d'une cellule périodique, elle même constituée d'un assemblage de résonateurs quarts d'onde en parallèle.
