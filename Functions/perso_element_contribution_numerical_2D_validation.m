function model = perso_element_contribution_numerical_2D_validation(element_contribution, env, varargin)
...
% couche d'air et d'une solution acoustique optimisée individuelle
    ...
    tp_config = element_contribution.Configuration.ListOfSubelements{1}.Configuration;
    ...
    % Air gap
    ag_config = element_contribution.Configuration.ListOfSubelements{2}.Configuration;
    ...
    % Sous-élement testé
    tested_element = element_contribution.Configuration.ListOfSubelements{3};
    model = tested_element.set_COMSOL_2D_Model(model, 1, env, '0', '-tpt -agt');
    ...
    % Pour la cavité
    box_cavity = model.component('component').selection.create('tested_element', 'Box');
    box_cavity.set('entitydim', 2); % On sélectionne les domaines
    box_cavity.set('ymax', '-tpt-agt+0.01[mm]');
    box_cavity.set('condition', 'inside');
    ...
    model = perso_add_selection_to_physics(model, 'phy_ap', 'air_gap');

    % On ajoute le reste de la solution à la Physique Thermo-Viscous Acoustic
    model = perso_add_selection_to_physics(model, 'phy_tv', 'tested_element');
