...
handle_config = @(x_TP) [radius_mm(x_TP(1))*1e-3, x_TP(2), x_TP(3)];
...
%% Importation des paramètres optimisés de la plaque Poly

% x_TP_Poly_optimized = load(uigetfile(folderName));
x_TP_Poly_optimized = [1, 8, 22];
% top_plate(handle_config(x_TP_Poly_optimized)).Configuration.Porosity;
...
x0_TP = horzcat(tp_r_init, tp_whn_init, tp_dhn_init);
x0 = horzcat(x0_TP, dw_init, pw_init);

%% Contrainte sur les variables entières

intcon = find(horzcat([1, 1, 1], zeros(1, N*NS), ones(1, N*NS)));
...
Objets.cell_of_MPPSBH_elements = @(x_ETS) arrayfun(@(i) ...
    Objets.MPPSBH_element_i(x_ETS, i), 1:NS ,'UniformOutput', false);
...
Contributions.contribution_MPPSBH_element_i = @(x_TP, x_ETS, i) classelement(classelement.create_config( ...
    {perso_modify_subelement_dimensions(top_plate(handle_config(x_TP)), ETS_cavities_width, ETS_cavities_depth), ...
     perso_modify_subelement_dimensions(air_gap, ETS_cavities_width, ETS_cavities_depth), ...
     classcavity(classcavity.create_config(ETS_cavities_thickness, ETS_cavities_width, ETS_cavities_depth)), ...
     Objets.MPPSBH_i(x_ETS, i)}, 'closed', ETS_input_surface));

Contributions.contribution_cell_of_MPPSBH_element = @(x_TP, x_ETS) arrayfun(@(i) ...
    Contributions.contribution_MPPSBH_element_i(x_TP, x_ETS, i), 1:NS ,'UniformOutput', false);

Contributions.contribution_Poly_element = @(x_TP) classelement(classelement.create_config( ...
    {perso_modify_subelement_dimensions(top_plate(handle_config(x_TP)), Poly_cavities_width, Poly_cavities_depth), ...
...
Contributions.contribution_Poly_numerical_element = @(x_TP) classelement(classelement.create_config( ...
    {perso_modify_subelement_dimensions(top_plate(handle_config(x_TP)), Poly_cavities_width, Poly_cavities_depth), ...
     perso_modify_subelement_dimensions(air_gap, Poly_cavities_width, Poly_cavities_depth), ...
     numerical_Poly_subelement}, 'closed', Poly_input_surface));

Contributions.cell_of_Poly_element_contribution = @(x_TP) arrayfun(@(i) ...
    Contributions.contribution_Poly_element(x_TP), 1:NS ,'UniformOutput', false);

Contributions.contribution_ETS_yellow_cavity = @(x_TP) classelement(classelement.create_config( ...
    {perso_modify_subelement_dimensions(top_plate(handle_config(x_TP)), yellow_cavities_width, yellow_cavities_depth), ...
...
Contributions.cell_of_ETS_yellow_cavity_contributions = @(x_TP) arrayfun(@(i) ...
    Contributions.contribution_ETS_yellow_cavity(x_TP), 1:4 ,'UniformOutput', false);

Contributions.contribution_Poly_yellow_cavity = @(x_TP) classelement(classelement.create_config( ...
    {perso_modify_subelement_dimensions(top_plate(handle_config(x_TP)), yellow_cavities_width, yellow_cavities_depth), ...
     perso_modify_subelement_dimensions(air_gap, yellow_cavities_width, yellow_cavities_depth), ...
     yellow_cavity}, 'closed', yc_input_surface));

Contributions.cell_of_Poly_yellow_cavity_contributions = @(x_TP) arrayfun(@(i) ...
    Contributions.contribution_Poly_yellow_cavity(x_TP), 1:4 ,'UniformOutput', false);
...
Modules = struct();
Modules.module_ETS = @(x_ETS) classelementassembly(classelementassembly.create_config( ...
        [Objets.cell_of_MPPSBH_elements(x_ETS), ... % Solutions ETS
            repmat({yellow_cavity_element}, 1, 4)])); % Solutions HR 

Modules.module_ETS_sans_HR = @(x_ETS) classelementassembly(classelementassembly.create_config(Objets.cell_of_MPPSBH_elements(x_ETS))); 

Modules.module_Poly = classelementassembly(classelementassembly.create_config( ...
                [repmat({imported_Poly_element}, 1, NS), ... % Solutions Poly
                    repmat({yellow_cavity_element}, 1, 4)])); % Solutions HR 

Modules.module_Poly_sans_HR = classelementassembly(classelementassembly.create_config(repmat({imported_Poly_element}, 1, NS)));

%% Création dynamique des cartouches

Cartouches = struct();

Cartouches.cartouche_ETS = @(x_TP, x_ETS) classelement(classelement.create_config( ...
    {top_plate(handle_config(x_TP)), air_gap, Modules.module_ETS(x_ETS)}, 'closed', total_input_surface));

Cartouches.cartouche_ETS_sans_HR = @(x_TP, x_ETS) classelement(classelement.create_config( ...
    {top_plate(handle_config(x_TP)), air_gap, Modules.module_ETS_sans_HR(x_ETS)}, 'closed', total_input_surface));

Cartouches.cartouche_ETS_contributions = @(x_TP, x_ETS) classelementassembly(classelementassembly.create_config( ...
    [Contributions.contribution_cell_of_MPPSBH_element(x_TP, x_ETS), Contributions.cell_of_ETS_yellow_cavity_contributions(x_TP)]));

Cartouches.cartouche_ETS_sans_HR_contributions = @(x_TP, x_ETS) classelementassembly(classelementassembly.create_config( ...
    Contributions.contribution_cell_of_MPPSBH_element(x_TP, x_ETS)));

Cartouches.cartouche_Poly = @(x_TP) classelement(classelement.create_config( ...
    {top_plate(handle_config(x_TP)), air_gap, Modules.module_Poly}, 'closed', total_input_surface));

Cartouches.cartouche_Poly_sans_HR = @(x_TP) classelement(classelement.create_config( ...
    {top_plate(handle_config(x_TP)), air_gap, Modules.module_Poly_sans_HR}, 'closed', total_input_surface));

Cartouches.cartouche_Poly_contributions = @(x_TP) classelementassembly(classelementassembly.create_config( ...
    [Contributions.cell_of_Poly_element_contribution(x_TP), Contributions.cell_of_Poly_yellow_cavity_contributions(x_TP)]));

Cartouches.cartouche_Poly_sans_HR_contributions = @(x_TP) classelementassembly(classelementassembly.create_config( ...
    Contributions.cell_of_Poly_element_contribution(x_TP)));

Cartouches.cartouche_globale = @(x) classelementassembly(classelementassembly.create_config({Cartouches.cartouche_ETS(x_TP(x), x_ETS(x)), Cartouches.cartouche_Poly(x_TP_Poly_optimized)}));
...
objective = @(x) [cost_function_obj0(x, env(dB)), cost_function_obj1(x, env(dB)), cost_function_obj2(x, env(dB)), cost_function_obj3(x, env(dB))];
...
temp_plot_cartouches = @(x) plot_cartouches(x_TP_Poly_optimized, x_TP(x), x_ETS(x), Cartouches, env(dB));
...
if env_saved == 1
    ...
    mkdir([folderName, '\optimisation_ETS_', currentTime]);
    save([folderName, '\optimisation_ETS_', currentTime '\environnement matlab.mat']);
else
    return
end
...
handle_config(x_TP(x_opti));
top_plate(handle_config(x_opti)).Configuration.Porosity

figure()
Cartouches.cartouche_globale(x_opti).plot_alpha(env(dB), 'Cartouche globale');
temp_plot_cartouches(x_opti);
saveas(gcf, [folderName, '\optimisation_ETS_', currentTime, '\Figures\Validation de la configuration optimale.fig']);
...
mphsave(Tube_MPPSBH.Configuration.ComsolModel, [folderName, '\optimisation_ETS_', currentTime, '\validation_2D_MPPSBH_', num2str(i), '.mph']);
mphgeom(Tube_MPPSBH.Configuration.ComsolModel)
...
for i = 1:NS
    figure()
    Tube_MPPSBH_element_contrib = ImpedanceTube2D(ImpedanceTube2D.create_config({Contributions.contribution_MPPSBH_element_i(x_TP(x_opti), x_ETS(x_opti), i)}));
    Tube_MPPSBH_element_contrib = Tube_MPPSBH_element_contrib.launch_tube_measurement(env(dB));
    Tube_MPPSBH_element_contrib.plot_alpha(env(dB), ['Contribution MPPSBH' num2str(i)]);
    perso_configure_alpha_figure(2000);
    saveas(gcf, [folderName, '\optimisation_ETS_', currentTime, '\Figures\Validation de la contribution de MPPSBH' num2str(i) '.fig']);
    
    figure()
    mphgeom(Tube_MPPSBH_element_contrib.Configuration.ComsolModel);
    saveas(gcf, [folderName, '\optimisation_ETS_', currentTime, '\Figures\Géométrie de l''élement MPPSBH' num2str(i) '.fig']);
    mphsave(Tube_MPPSBH_element_contrib.Configuration.ComsolModel, [folderName, '\optimisation_ETS_', currentTime, '\validation_2D_MPPSBH_', num2str(i), '.mph']);
end

figure()
Tube_ETS_yc_contrib = ImpedanceTube2D(ImpedanceTube2D.create_config({Contributions.contribution_ETS_yellow_cavity(x_opti)}));
Tube_ETS_yc_contrib = Tube_ETS_yc_contrib.launch_tube_measurement(env(dB));
Tube_ETS_yc_contrib.plot_alpha(env(dB), 'Contribution ETS cavité jaune');
perso_configure_alpha_figure(2000);
saveas(gcf, [folderName, '\optimisation_ETS_', currentTime, '\Figures\Validation de la contribution de la cavité jaune.fig']);
mphsave(Tube_ETS_yc_contrib.Configuration.ComsolModel, [folderName, '\optimisation_ETS_', currentTime, '\validation_2D_ETS_yellow_cavity.mph']);
