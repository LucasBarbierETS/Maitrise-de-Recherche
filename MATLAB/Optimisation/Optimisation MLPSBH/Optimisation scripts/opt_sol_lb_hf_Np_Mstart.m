...
x0_to_MPPSBH_i = @(x0, i) classMPPSBH_Rectangular(classMPPSBH_Rectangular.create_explicit_config(N, cavities_depth, cavities_width, ...
                                                                                                ...
                                                                                                {eval_r(fix(x0(:, 1, i)))}, ...  % rayon des perforations
                                                                                                {transpose(x0(:, 2, i))}, ... % distance entre les perforations dans le sens de la largeur
                                                                                                {transpose(fix(x0(:, 3, i)))}, ... % nombre de perforations dans le sens de la profondeur
                                                                                                {transpose(fix(x0(:, 4, i)))}, ... % nombre de perforations dans le sens de la largeur
...
options = optimoptions('ga', ...
    ...
    'EliteCount', 5, ...
    'MigrationInterval', 10, ... % Nombre de générations avant migration
    'MigrationFraction', 0.2, ... % Fraction d'individus qui migrent
    'MigrationDirection', 'both', ...
    'PopulationType', 'doubleVector', ...
    'Subpopulation', 5); % Nombre de sous-populations

% Exécution de l'algorithme multi-GA

rng; % Pour la reproductibilité
tic;
[x_opt, fval, exitflag, output, population, scores] = ga(objective, numel(x0), [], [], [], [], lb, ub, handle_perso_nonlcon, intcon, options);
timeGa = toc;

%% Résultats

% Solutions et configurations optimisée
assembly_lb_hf_opti = params_to_MPPSBH_assembly(reshape(x_opt_lb_hf, N, NV, NS));
