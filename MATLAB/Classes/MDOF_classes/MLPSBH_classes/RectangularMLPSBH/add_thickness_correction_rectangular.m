function surconfig = add_thickness_correction_rectangular(config)
...
SBHl = config.SBHLength;
SBHw = config.SBHWidth;
ppal = config.PlatesPerforatedAreaLength;
ppaw = config.PlatesPerforatedAreaWidth;
pp = config.PlatesPorosity;
ct = config.CavitiesThickness;
hr = config.PlatesHolesRadius;

% Initialisation du vecteur des corrections d'épaisseur
ptc = zeros(size(ppal));
    ...
    case 'Bezançon' % used in [2]    ordre de grandeur : e-2
        ...
        phi = pp(1) * ppal(1) * ppaw(1) / (SBHl * SBHw);                     % Porosité réelle de la plaque 
        S = pp(1) * ppal(1) * ppaw(1);                                     % Surface d'ouverture réelle de la plaque
        correction_length = 0.48 * sqrt(S) * (1 - 1.14 * sqrt(phi));  % Longueur de correction de rayonnement. [1] eq. (9.18) p. 191
    ...
    case 'Beranek' % Beranek Insgard % [1] eq.10 p.5
        ...
        for i = 1:length(ptc)
            phi = pp(1) * ppal(1) * ppaw(1) / (SBHl * SBHw);
            ptc(i) = 0.48 * sqrt(pi * hr(i)^2) * (1 - 1.47 * sqrt(phi) + 0.47 * sqrt(phi));   
        end

    case 'Allard'  % Allard Insgard % [1] eq.9 p.4
       
        for i = 1:length(ptc)
            phi = pp(1) * ppal(1) * ppaw(1) / (SBHl * SBHw);
            ptc(i) = 2 * 0.48 * sqrt(pi * hr(i)^2) * (1 - 1.14 * sqrt(phi));  
        end
    end 
