...
comp.physics('acpr').selection.named('tube_sel');  % Sélection du tube d'impédance

% Création d'une physique thermoacoustique
comp.physics.create('ta', 'ThermoacousticsSinglePhysics', 'geom1'); 
comp.physics('ta').selection.named('mat_sel');  % Sélection du matériau

% Création d'une fonctionnalité de pression dans la physique acoustique
comp.physics('acpr').create('pr1', 'Pressure', 1); 
comp.physics('acpr').feature('pr1').selection.named('source_bnd');  % Sélection du plan de la source
...
comp.multiphysics('atb1').selection.named('tva_bnd'); % Sélection de la frontière TVA
...
comp.mesh('mesh1').feature('size').set('hauto', 3);  
% Définition de l'angle de division pour la couche de bord
comp.mesh('mesh1').feature('bl1').set('splitdivangle', 20);  
...
comp.mesh('mesh1').feature('bl1').feature('blp').set('blstretch', 1);  
...
model.result.numerical('av1').selection.named('mic2_bnd');  
