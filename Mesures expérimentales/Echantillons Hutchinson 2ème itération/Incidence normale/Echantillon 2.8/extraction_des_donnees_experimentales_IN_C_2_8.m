folder_path = [env.Root, '\Mesures expérimentales\Echantillons Hutchinson 2ème itération\Incidence normale\Echantillon 2.8'];

% Configuration C2_8

config_2_8_PW = classMPPSBH_Rectangular.create_explicit_slit_pattern_config(28e-3^2, 5, 28e-3, 28e-3, ...
    {5.9531e-04}, ...
...
config_2_8_PWC = classMPPSBH_Rectangular.create_explicit_slit_pattern_config(28e-3^2, 5, 28e-3, 28e-3, ...
    {5.9531e-04}, ...
    {[0.0024 0.0024 0.0024 0.0024 0.0025]}, ...
    {[0.0028 0.0028 0.0028 0.0028 0.0028]}, ...
    {10}, ...
...
config_2_8_LV = classMPPSBH_Rectangular.create_explicit_slit_pattern_config(28e-3^2, 5, 28e-3, 28e-3, ...
    {5.9531e-04}, ...
    {[0.0024 0.0024 0.0024 0.0024 0.0025]}, ...
    {[0.0028 0.0028 0.0028 0.0028 0.0028]}, ...
    {10}, ...
...
MPPSBH_PW = classMPPSBH_Rectangular_iter2(config_2_8_PW);
MPPSBH_PWC = classMPPSBH_Rectangular_iter2(config_2_8_PWC);
MPPSBH_LV = classMPPSBH_Rectangular_iter2(config_2_8_LV);

% perso_figure('Validation expérimentale - C2.8 - 100 dB'); hold on % Affichage individuel
perso_figure('Comparaison expérimental - numérique'); subplot(4, 2, 8); hold on % Affichage groupé
...
Tube2D_tv.plot_alpha('C2.8 num. 2D');
...
Tube3D_ap.plot_alpha('C2.8 num. 3D');
...
%                                            'Incidence normale\Echantillon 2.8\Export_Data_8']);
% 
% plot(data.f, data.alpha.Sample1, 'DisplayName', 'Mesure expérimentale');

N = 8; % Mesure associée à l'échantillon 2.8
...
% Courbe moyenne
plot(f, m, 'b', 'DisplayName', 'C2.8 exp.', 'LineWidth', 0.5);
