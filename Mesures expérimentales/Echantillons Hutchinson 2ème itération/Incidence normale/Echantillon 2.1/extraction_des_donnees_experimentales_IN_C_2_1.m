folder_path = [env.Root, '\Mesures expérimentales\Echantillons Hutchinson 2ème itération\Incidence normale\Echantillon 2.1'];

% Configuration C2_1

config_2_1_PW = classMPPSBH_Rectangular.create_explicit_slit_pattern_config(28e-3^2, 5, 28e-3, 28e-3, ...
    {5.9531e-04}, ...
...
config_2_1_PWC = classMPPSBH_Rectangular.create_explicit_slit_pattern_config(28e-3^2, 5, 28e-3, 28e-3, ...
    {5.9531e-04}, ...
    {[0.0025 0.0024 0.0024 0.0025 0.0025]}, ...
    {[0.0028 0.0028 0.0028 0.0028 0.0028]}, ...
    {10}, ...
...
config_2_1_LV = classMPPSBH_Rectangular.create_explicit_slit_pattern_config(28e-3^2, 5, 28e-3, 28e-3, ...
    {5.9531e-04}, ...
    {[0.0025 0.0024 0.0024 0.0025 0.0025]}, ...
    {[0.0028 0.0028 0.0028 0.0028 0.0028]}, ...
    {10}, ...
...
MPPSBH_PW = classMPPSBH_Rectangular_iter2(config_2_1_PW);
MPPSBH_PWC = classMPPSBH_Rectangular_iter2(config_2_1_PWC);
MPPSBH_LV = classMPPSBH_Rectangular_iter2(config_2_1_LV);

% perso_figure('Validation expérimentale - C2.1 - 100 dB'); hold on % Affichage individuel
perso_figure('Comparaison expérimental - numérique'); subplot(4, 2, 1); hold on % Affichage groupé
...
plot(env.w/(2*pi), MPPSBH_LV.alpha(env), 'DisplayName', 'Modèle analytique linéaire - AV');
...
Tube2D_tv.plot_alpha('C2.1 num. 2D');
...
Tube3D_ap.plot_alpha('C2.1 num. 3D');
...
%                                            'Incidence normale\Echantillon 2.1\Export_Data_1']);
% 
% plot(data.f, data.alpha.Sample4, 'DisplayName', 'C2.1 exp.');
...
N = 9; % Mesure associée à l'échantillon 2.2
...
% Courbe moyenne
plot(f, m, 'b', 'DisplayName', 'C2.2 exp.', 'LineWidth', 0.5);
xlim([data.f(1) data.f(end)])
...
% 
% perso_figure('Comparaison des revêtements')
% hold on
% % plot(data.f, data.alpha.Sample1, '--', 'DisplayName', 'Mur rigide');
% plot(data.f, data.alpha.Sample2, '--', 'DisplayName', 'Term. Anéchoique');
% plot(data.f, data.alpha.Sample3, 'LineWidth', 1, 'DisplayName', 'T.A Tape électrique');
% plot(data.f, data.alpha.Sample4, 'LineWidth', 1, 'DisplayName', 'T. A. Leak Seal');
